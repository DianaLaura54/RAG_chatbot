...
class RAGDataValidator:
    ...
    def validate_pdf_processing_results(self, documents_with_pages: List[List[Tuple]],
                                        file_sources: List[str]) -> Dict[str, Any]:
    ...
    def validate_search_results(self, search_results: List[Dict], query: str,
...
def validate_and_process_documents(folder_path: str, validator: RAGDataValidator = None,
    ...
    def validate_full_pipeline(folder_path: str, query: str, expected_results: int = 5):
        ...
        try:
            ...
            if index and bm25_model:
                ...
                bm25_results = bm25_search(bm25_model, tokenized_corpus, texts, metadata, query,
                                           n_results=expected_results)
                hybrid_results = hybrid_search(index, bm25_model, tokenized_corpus, texts, metadata, query,
                                               n_results=expected_results)
...
def integrate_validation_with_testing():

    class ValidatedSingleQuestionTester:

        def __init__(self, base_path: str = "Contents"):
            self.base_path = base_path
            self.validator = RAGDataValidator(base_path)

        def run_validated_test(self, question: str, expected_answer: str):
            logger.info("Starting validated test run")
            folder_path = os.path.join(self.base_path, "books")
            processing_results = validate_and_process_documents(folder_path, self.validator)
            if not processing_results:
                logger.error("Test aborted due to processing validation failure")
                return None
            return {
                'processing_validation': processing_results['validation_results'],
                'validation_report': processing_results['validation_report']
